...
namespace ts.refactor.convertStringOrTemplateLiteral {
    ...
    function concatConsecutiveString(index: number, nodes: readonly Expression[]): [number, string, string, number[]] {
        const indexes = [];
        let text = "", rawText = "";
        while (index < nodes.length) {
            const node = nodes[index];
            if (isStringLiteralLike(node)) { // includes isNoSubstitutionTemplateLiteral(node)
                text = text + escapeStringForTemplate(node.text);
                rawText = rawText + escapeStringForTemplate(getTextOfNode(node).slice(1, -1));
                indexes.push(index);
                index++;
            }
            else if (isTemplateExpression(node)) {
                text = text + node.head.text;
                rawText = rawText + getRawTextOfTemplate(node.head);
                break;
            }
            else {
                break;
        ...
        for (let i = begin; i < nodes.length; i++) {
            ...
            if (isTemplateExpression(currentNode)) {
                const spans = map(currentNode.templateSpans, (span, index) => {
                    ...
                    const rawText = getRawTextOfTemplate(span.literal) + (nextSpan ? "" : rawSubsequentText);
